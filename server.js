  ...
  if(socket.handshake.headers.origin != system.url)
	...
	if (!interval) 
	{
		var ducnghiaload = setInterval(function()
		{
			...
			{
				/*Auto thua nếu chỉ có một người chơi*/
				var sltai = checknguoi('tai000');
				var slxiu = checknguoi('xiu0000');
				if(sltai == 0 && slxiu !=0) //có xỉu nhưng k tài => tài
				{
					if(+game.x2 <=0)
					{
						game.x2 = rand(4,6);
				...
				if(sltai != 0 && slxiu ==0) //Có tài nhưng k xỉu => xỉu
				{
					if(+game.x2 <=0)
					{
						game.x2 = rand(1,3);
